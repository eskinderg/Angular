...
describe('triggers', () => {
    ...
    describe('listenToTriggers', () => {
        ...
        let states: boolean[] = [];
        const isOpenedFn = () => (states.length === 0 ? false : states[states.length - 1]);
        const openFn = jasmine.createSpy('openFn').and.callFake(() => states.push(true));
        const closeFn = jasmine.createSpy('closeFn').and.callFake(() => states.push(false));
        let cleanupFn: () => void;
